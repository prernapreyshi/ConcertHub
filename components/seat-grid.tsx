...
export function SeatGrid({ onSeatClick, disabled }: SeatGridProps) {
  ...
  // 🔥 Convert "A1" → row A, number 1
  const parseSeat = (seat: Seat) => ({
    row: seat.seatId[0],
    number: parseInt(seat.seatId.slice(1)),
  })
  ...
  const getSeatClasses = (seat: Seat) => {
    ...
    switch (seat.status) {
      ...
      case "selected":
        return cn(
          base,
          "bg-seat-locked text-muted-foreground cursor-pointer"
        )
      case "locked":
        return cn(
          base,
  ...
  const handleSeatClick = (seat: Seat) => {
    if (disabled) return
    if (seat.status === "available" || seat.status === "selected" || seat.status === "locked") {
      onSeatClick(seat)
    }
  }
          ...
          {rows.map(row => (
            <div key={row} className="flex items-center gap-1 sm:gap-2">
              ...
              <div className="flex gap-1 sm:gap-1.5">
                {seatsByRow[row]
                  .sort((a, b) => parseSeat(a).number - parseSeat(b).number)
                  .map((seat, index) => {
                    const { number } = parseSeat(seat)
